...
function exolib.closestVal(nums, number)
    ...
    for i, num in ipairs(nums) do
        if tonumber(num) == nil then print("EXOLIB ERROR: Table given to closestVal() must contain ONLY numbers.") return nil
        elseif tonumber(number) == nil then print("EXOLIB ERROR: Number given to closestVal() MUST be a number") return nil
        else
...
function exolib.getBelow(nums, number, equal) -- if equal is true, then it will return numbers equal to the number argument
    ...
    for i, num in ipairs(nums) do
        ...
        elseif tonumber(number) == nil then print("EXOLIB ERROR: All variables given to closestVal() MUST be numeric.") return nil
        else
